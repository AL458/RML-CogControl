...
Centered_optimum = [0.227,0.992,0.245,0.259,31.25]; % Optimized value of the parameters as determined by the gradient descent algorithm.
...
for par = 1:length(locs)
    for valuenr = 1:length(values)
        ...
        % Invert the vrt parameter so it can be used for simulation
        Changed_optimum(5)=1/Changed_optimum(5);
