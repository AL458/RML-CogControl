function [W,We] = Run_training_DAN(n_training_trials,arg,seed,optimization_pars,save_subject_data)
% Modified from the RML function generating the responses for one
% simulated participant
% Runs the training for the RML-DAN. Trains the AccAct and AccBoost module
    ...
    for state = 1:nstate
        for boost = 1:size(arg.constBoost.costs,2)
            ...
            while s<=nstate %within trial state transitions
                ...
                Vdiff = AccAct.V(s,1)-AccAct.V(s,2);                % Get the difference in value
                RT_baseline = arg.rt_baseline_dualattr+optimization_pars.dualattractor.bias; % This line allows for optimization of the RT_baseline value, but is currently not used
                % Run the dual attractor network
                [ev_arr,RT,res] = get_RT_dual_attractor_improved(Vdiff,b,optimization_pars,arg.rt_timeout-RT_baseline); 
                RT = RT+RT_baseline;
                if res~=0
                    ...
                    % If the participant is late, no reward is given
                    rw = 0;
                    RW = 0;
                    s1 = 37;
                end
