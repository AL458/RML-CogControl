%response analysis
function [opt, rw, RW,s1]=resp_analys(trin,se,miniblock,s,sterminal,resp,rwlist,rwlistm,RWM,RWP,punish,trans,chance)
...
if resp<3 %if decides to engage in the task
    ...
    if sum(RWM(s,resp+(se-1)*2))~=0 %if there is a primary reward in state s
        if se==1%if Stat
            ...
            rw=rwlist(s,resp+(se-1)*2,trin);
            RW=rwlistm(s,resp+(se-1)*2,trin);
            if rw==0 && punish~=0 %if punish
               RW=-rwlistm(s,resp+(se-1)*2,trin)*punish;
               rw=ceil(punish);
            end
        end
        ...
        if se==3%if Vol--------------------------------------------------------------
            ...
            if sum(miniblock==[1 3 5])>=1%if it's the second or the fourth or fifth
                ...
                if (RWM(s,resp+(se-1)*2)*RWP(s,resp+(se-1)*2))==max(RWM(s,[1:2]+(se-1)*2).*RWP(s,[1:2]+(se-1)*2))%pascalian optimum (inverted due to Vol)                   
                    opt=1;%otpimality 1=yes
                else
                    opt=0;
                end
