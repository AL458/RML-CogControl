...
Centered_optimum = [40.83, 33.39, 0.39, 28.96];% Optimized value of the parameters as determined by the gradient descent algorithm.
...
currfolder = pwd;
addpath(currfolder)
addpath(fullfile('..','DDM_scripts','Scripts'))
addpath(fullfile('..','DDM_scripts','Scripts','All_scripts'))
addpath(fullfile('..','DDM_scripts','Input'))
load('W3.mat')
load('We3.mat')
...
for par = 1:length(locs)
    for valuenr = 1:length(values)
        ...
        [dat,output,optimization_pars] = RML_main_opt_DDM_function(nsubj,Changed_optimum,W,We);
...
rmpath .
rmpath(fullfile('..','DDM_scripts','Scripts'))
rmpath(fullfile('..','DDM_scripts','Scripts','All_scripts'))
rmpath(fullfile('..','DDM_scripts','Input'))
